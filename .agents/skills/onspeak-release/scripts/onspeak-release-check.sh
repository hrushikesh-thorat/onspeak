...
if ! .github/scripts/changelog-section.sh "$version" >/tmp/onspeak-release-notes.md; then
  echo "Could not extract CHANGELOG.md section for $version" >&2
  exit 1
fi

if [[ ! -s /tmp/onspeak-release-notes.md ]]; then
  echo "Extracted changelog section is empty for $version" >&2
  exit 1
fi

if ! grep -q "^## \\[$version\\]" /tmp/onspeak-release-notes.md; then
...
echo
cat /tmp/onspeak-release-notes.md
